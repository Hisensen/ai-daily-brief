...
3. 预取趋势项目 README，然后让 Codex 在无 Git、无 GitHub 网络权限的临时目录里只生成 `index.html`；临时 HOME 只链接登录凭证，不加载 `~/.claude` 技能或配置。
4. 校验日期、条数、全部摘要和来源链接、GitHub 项目与中文说明、搞钱参考和历史存档。
5. 在基于远端 `main` 的临时 worktree 中只提交 `index.html` 与当日 archive，再核对推送前后 SHA、提交 blob、远端归档内容和 GitHub Pages 实际页面。

用户当前工作区中的修改、未跟踪文件和 `.DS_Store` 不会被提交或删除。任何一步失败都会返回非零退出码、保留源简报并写入状态文件；若代码已推送但 Pages 尚未刷新，状态会明确记为 `PUSHED_NOT_LIVE`，补偿任务会继续复核。
